...
if (!requireNamespace("ggplot2", 
                      quietly = TRUE)) install.packages("ggplot2")
if (!requireNamespace("RColorBrewer", 
                      quietly = TRUE)) install.packages("RColorBrewer")
library(ggplot2)
...
read_fasta_alignment <- function(path) {
  ...
  for (ln in lines) {
    if (nchar(ln) == 0) next
    if (substr(ln,1,1) == ">") {
      if (!is.null(cur_name)) { names <- c(names, 
                                           cur_name); 
      seqs <- c(seqs, 
                paste0(cur_seq,
                       collapse = "")) }
...
extract_nums <- function(x) {
  m1 <- regmatches(x, 
                   regexpr("[0-9]+",
                           x))
  main_num <- ifelse(lengths(m1) == 0,
                     NA,
                     as.numeric(m1))
  m2 <- regmatches(x,
                   regexpr("_[0-9]+", 
                           x))
  sub_num <- ifelse(lengths(m2) == 0,
                    NA,
                    as.numeric(sub("_",
                                   "", 
                                   m2)))
...
mito_mask <- grepl("mitochondrion",
                   names(seqs), 
                   ignore.case = TRUE)
non_mito_names <- names(seqs)[!mito_mask]
if (length(non_mito_names) > 0) {
  df_nums <- extract_nums(non_mito_names)
  df_nums$main_num_for_sort <- ifelse(is.na(df_nums$main_num),
                                      .Machine$integer.max - 1, 
                                      df_nums$main_num)
  df_nums$sub_num_for_sort  <- ifelse(is.na(df_nums$sub_num), 
                                      -1, 
                                      df_nums$sub_num)
  df_nums <- df_nums[order(df_nums$main_num_for_sort,
                           df_nums$sub_num_for_sort,
                           df_nums$name), ]
  non_mito_sorted <- df_nums$name
} else { non_mito_sorted <- character(0) }

order_names <- c(non_mito_sorted,
                 names(seqs)[mito_mask])
...
aln_df$gene_fill <- ifelse(aln_df$is_gap,
                           NA, 
                           aln_df$gene)
...
if (n_genes <= 12) {
  base_pal <- brewer.pal(max(3,
                             n_genes),
                         "Paired") 
  pal <- colorRampPalette(base_pal)(n_genes)
} else {
  pal <- colorRampPalette(brewer.pal(12, 
                                     "Paired"))(n_genes)
}
names(pal) <- genes

aln_df$seq <- factor(aln_df$seq, 
                     levels = rev(names(seqs)))

p <- ggplot(aln_df, 
            aes(x = pos,
                        y = seq)) +
  geom_tile(aes(fill = gene_fill),
            color = NA, 
            height = 0.8) +  ## відступ між рядками
  scale_fill_manual(values = pal, 
                    na.value = "white",
                    name = "Gene") +
  geom_vline(xintercept = seq(0, 
                              aln_len,
                              by = 500), 
             color = "gray90",
             size = 0.2) +
  geom_text(data = data.frame(seq = levels(aln_df$seq)),
            aes(x = aln_len + round(aln_len*0.02),
                y = seq,
                label = ifelse(grepl("R_",
                                     as.character(seq), 
                                     ignore.case = TRUE),
                               "←", 
                               "→")),
